...
class Motor():
    ...
    def moveForward(self, speed, t=0): # 'speed' allows the user to input speed
        GPIO.output(self.In1, GPIO.LOW)
        GPIO.output(self.In2, GPIO.HIGH)
        self.pwm2.ChangeDutyCycle(speed)
        sleep(t)                        # delay
        
    def moveBackward(self, speed, t=0): # 'speed' allows the user to input speed
...
while True:
    ...
    #print("Ready to accept input from keypad")
    received_data = port.readline (1)              #read serial port
    sleep(0.03)
    data_left = port.inWaiting()             #check for remaining byte
    received_data += port.read(data_left)   
    #print (received_data)                   #print received data USED FOR DEBUG 
    print("Data received")
    
    if (received_data == b'b'):
        ...
        if ((received_data2 == b'o') and (lock == 1)):
            motor3.moveForward(100, 8)
            motor3.stop()
            
        elif ((received_data2 == b'o') and (lock == 0)):
            print("Cannot open panel while front is retracted")
            
        elif ((received_data2 == b'c') and (lock == 0)):
            print("Panel is already closed")
